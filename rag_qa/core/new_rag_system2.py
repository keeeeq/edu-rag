...
class RAGSystem:
    ...
    def _retrieve_with_backtracking(self, query, source_filter):
        ...
        try:
            #   调用大语言模型生成回溯问题
            simplified_query = self.llm_call_dashscope(backtrack_prompt_template.format(query=query)).strip()
    ...
    def _retrieve_with_subqueries(self, query, source_filter):
        ...
        try:
            #   调用大语言模型生成子查询列表
            subqueries_text = self.llm_call_dashscope(subquery_prompt_template.format(query=query)).strip()
    ...
    def _retrieve_with_hyde(self, query, source_filter):
        ...
        #   调用大语言模型生成假设答案
        try:
            hypo_answer = self.llm_call_dashscope(hyde_prompt_template.format(query=query)).strip()
    ...
    def generate_answer(self, query, source_filter=None, history=None):
        ...
        try:
            # 使用大模型获得输出结果：
            response = self.llm_call_dashscope_stream(prompt_input)
    ...
    def llm_call_dashscope(self, prompt):
        # 调用 DashScope API
        try:
    ...
    def llm_call_dashscope_stream(self, prompt):
